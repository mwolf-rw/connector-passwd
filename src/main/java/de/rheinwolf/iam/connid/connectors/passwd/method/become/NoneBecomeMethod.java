...
/**
 * Pseudo-method for directly passing on commands, assuming the current user is already privileged.
 */
public class NoneBecomeMethod implements BecomeMethod {
    /**
     * Executing a command.
     *
    ...
    public CommandResult execute(Connection connection, byte[] stdin, String command, String... args) {
        return connection.execute(stdin, Stream.concat(
                Arrays.stream(new String[]{command}),
                Arrays.stream(args)
        ).toArray(String[]::new));
    }
